...
!function (t) {
    function i(i, s) {
        this.container = t(i), this.settings = s, this.timer = 0, this.before = {
            v: 0,
            h: 0
        }, this.touch = {}, this.pressed = 0, this.vslider = t("<div/>", {"class": "rollbar-handle"}), this.vpath = t("<div/>", {"class": "rollbar-path-vertical"}), this.hslider = t("<div/>", {"class": "rollbar-handle"}), this.hpath = t("<div/>", {"class": "rollbar-path-horizontal"}), this.sliders = this.vslider.add(this.hslider), this.container.css({
            position: "relative",
            overflow: "hidden"
        }).contents().filter(this.settings.contentFilter).wrapAll('<div class="rollbar-content"></div>'), this.content = this.container.children(".rollbar-content").css({
            position: "relative",
            top: 0,
            left: 0,
            overflow: "hidden"
            ...
            this.checkScroll(), this.pathSize()
    ...
    i.prototype.checkScroll = function () {
        ...
        this.vpath.css({top: t + "px", height: this.container.height() - 2 * t + "px"}), this.hpath.css({
            left: t + "px",
        ...
        var e = 0, h = Math.floor(this.settings.scrollTime / this.settings.scrollInterval), n = this.vslider.position().top, o = this.hslider.position().left, r = t.easing[this.settings.scrollEasing] || t.easing.linear;
        ...
        this.bindEvent(this.sliders, "mousedown", function (t) {
            ...
            var s = t.pageX, e = t.pageY, h = this.vslider.position().top, n = this.hslider.position().left;
            ...
            var i = t.originalEvent, s = i.changedTouches[0];
            this.touch.sx = s.pageX, this.touch.sy = s.pageY, this.touch.sv = this.vslider.position().top, this.touch.sh = this.hslider.position().left, this.sliders.stop().fadeTo(this.settings.sliderOpacityTime, 1), this.settings.blockGlobalScroll && (this.vdiff || this.hdiff) && i.stopPropagation()
        }), this.bindEvent(this.container, "touchmove", function (t) {
            var i = t.originalEvent, s = i.targetTouches[0];
            this.scroll(this.touch.sv + (this.touch.sy - s.pageY) * this.settings.touchSpeed, this.touch.sh + (this.touch.sx - s.pageX) * this.settings.touchSpeed, t), this.settings.blockGlobalScroll && (this.vdiff || this.hdiff) && (i.preventDefault(), i.stopPropagation())
        }), this.bindEvent(this.container, "touchend touchcancel", function (t) {
            {
                var i = t.originalEvent;
                i.changedTouches[0]
            }
            this.sliders.stop().fadeTo(this.settings.sliderOpacityTime, this.settings.sliderOpacity), this.settings.blockGlobalScroll && (this.vdiff || this.hdiff) && i.stopPropagation()
        });
        var s = this.vpath.height(), e = this.hpath.width();
        this.bindEvent(t(window), "resize", function () {
            ...
            var i = 0, s = 0;
            s = 38 == t.keyCode ? -this.settings.keyScroll : s, s = 40 == t.keyCode ? this.settings.keyScroll : s, i = 37 == t.keyCode ? -this.settings.keyScroll : i, i = 39 == t.keyCode ? this.settings.keyScroll : i, (s || i) && (this.easeScroll(s, i), t.preventDefault())
        }), this.bindEvent(this.container, "dragstart", function (t) {
            t.preventDefault()
        }), this.bindEvent(this.container, "rollbar", function (t, i, s, e) {
            return t.stopPropagation(), "reset" === i ? void this.container.find(".rollbar-content, .rollbar-handle").css({
                top: 0,
                left: 0
